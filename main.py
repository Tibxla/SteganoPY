...
def encode_file_in_image(image_path, file_to_hide_path, target_bit):
    ...
    # Retourner l'image encodée sous forme de buffer en mémoire
...
def decode_file_from_image(image_path):
    ...
    hidden_data_size = width * height - 1
    ...
    if 4 + hidden_data_length <= len(hidden_data):
        ...
        try:
            ...
            return None, None
        # Déterminer l'extension du fichier à partir du contenu (magic bytes)
        file_extension = ""
        if hidden_data.startswith(b"\x89PNG"):
            ...
            # Vérifier si les données décodées sont du texte UTF-8 lisible
            try:
                hidden_data.decode("utf-8")
                file_extension = ".txt"
            except UnicodeDecodeError:
                print("Le type de fichier caché n'est pas pris en charge.")
                return None, None

        # Retourner le contenu en mémoire avec son extension
        output_buffer = io.BytesIO(hidden_data)
...
def encode():
    if request.method == 'POST':
        ...
        tmp_file_path = None
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.tmp') as tmp_image:
                tmp_image_path = tmp_image.name
                image_file.save(tmp_image_path)

            with tempfile.NamedTemporaryFile(delete=False, suffix='.tmp') as tmp_file:
                tmp_file_path = tmp_file.name
                file_to_hide.save(tmp_file_path)

            output_buffer = encode_file_in_image(tmp_image_path, tmp_file_path, target_bit)
            if output_buffer is not None:
                download_name = f"{output_name}.png" if output_name else "encoded_image.png"
                return send_file(output_buffer, mimetype='image/png', as_attachment=True, download_name=download_name)
            else:
...
def decode():
    if request.method == 'POST':
        ...
        try:
            ...
            output_buffer, file_extension = decode_file_from_image(tmp_image_path)

            if output_buffer is not None:
                output_name = request.form.get('output_name')
                download_name = (
                    f"{output_name}{file_extension}" if output_name and file_extension
                    else f"decoded_file{file_extension}" if file_extension
                    else "decoded_file"
                )
                response = make_response(
                    send_file(output_buffer, as_attachment=True, download_name=download_name)
                )
                response.headers.add('X-Decoded-File-Extension', str(file_extension) if file_extension else '')
                return response
            else:
